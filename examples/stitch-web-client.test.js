var WebClient = require('./stitch-web-client');
var Connection = require('mongodb-connection-model');
var stitchConnection = new Connection({
  hostname: '127.0.0.1',
  port: 8080,
...
describe('WebClient', function() {
  ...
  this.timeout(20000);
  let userId;
  var client = new WebClient(stitchConnection);
  ...
  describe.skip('#count', function() {
    ...
    it('yields the error and the count', function(done) {
      client.count('data-service.test', { owner_id: userId }, {}, function(
        error,
        count
      ) {
        expect(error).to.equal(null);
  ...
  describe.skip('#find', function() {
    ...
    it('yields the error and the results to the callback', function(done) {
      client.find('data-service.test', { owner_id: userId }, {}, function(
        error,
        results
      ) {
        expect(error).to.equal(null);
