...
class NativeClient {
   ...
   * Get the currentOp.
   *
   * @param {Integer} include_all - if true also list currently idle operations in the result.
   * @param {Function} callback - The callback.
   */
  currentOp(include_all, callback) { // TODO: deal with db versions? && do we want operations arg?

    this.database.admin().command(
        {'currentOp': 1, '$all': include_all}, (error, result) => {
      if (error) {
        this._database('admin').collection('$cmd.sys.inprog').findOne(
            {'$all': include_all}, (error2, result2) => {
          if (error2) {
