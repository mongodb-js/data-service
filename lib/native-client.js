...
class NativeClient {
   ...
   * @param {Function} callback - The callback.
   */
  currentOp(operations, callback) { //TODO: deal with db versions?
    var db = this._database("admin"); //TODO: deal with permissions?
    // db.command({ "currentOp": operations }, (error, result) => {
    //   if (error) {
    //     return callback(this._translateMessage(error));
    //   }
    //   callback(null, result);
    // });
    db.collection("$cmd.sys.inprog").findOne({}, (error, result) => {
      if (error) {
        return callback(this._translateMessage(error));
      }
      callback(null, result);
