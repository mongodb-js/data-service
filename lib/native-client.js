...
class NativeClient {
   ...
   * Get the currentOp.
   *
   * @param {Object} operations - Document containing the operations to return information on TODO: optional?
   * @param {Function} callback - The callback.
   */
  currentOp(operations, callback) { // TODO: deal with db versions? is there a node driver op
    operations = operations || {};
    if (false) { // TODO
      this._admin().command({'currentOp': operations}, (error, result) => {
        if (error) {
          return callback(this._translateMessage(error));
        }
      ...
      this._database('admin').collection('$cmd.sys.inprog').findOne({operations}, (error, result) => {
        if (error) {
          return callback(this._translateMessage(error));
        }
        callback(null, result);
  ...
  serverStats(callback) {
    this._admin().serverStatus((error, result) => {
      if (error) {
        return callback(this._translateMessage(error));
      }
      callback(null, result);
   ...
   */
  top(callback) {
    this._admin().command({ 'top': 1 }, (error, result) => {
      if (error) {
        return callback(this._translateMessage(error));
      }
      callback(null, result);
  ...
  /**
   * Get the admin database. TODO: probably handle auth errors here?
   *
   * @returns {adminDB} The admin database object.
   */
  _admin() {
    return this.database.admin();
  }
