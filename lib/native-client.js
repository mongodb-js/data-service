...
class NativeClient extends EventEmitter {
  ...
  /**
   * Get command line options on the admin database.
   *
   * @param {Function} callback - The callback.
   */
  getCmdLineOpts(callback) {
    this.database.admin().command({getCmdLineOpts: 1}, (error, result) => {
      if (error) {
        return callback(this._translateMessage(error));
      }
      callback(null, result);
  ...
  replSetGetStatus(callback) {
    this.database.admin().command({replSetGetStatus: 1 }, (error, result) => {
      if (error) {
        return callback(this._translateMessage(error));
      }
      callback(null, result);
